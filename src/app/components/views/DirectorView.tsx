...
import { AlertCircle, CheckCircle2, UserPlus, CreditCard, ChevronRight, Building, Plus, Loader2, Clock, Trash2 } from 'lucide-react';
...
export default function DirectorView() {
  ...
  const [schoolCode, setSchoolCode] = useState(''); // Could be shortid from DB, using ID for now
  ...
  const [isSubscribingChoreography, setIsSubscribingChoreography] = useState(false);
  ...
  const fetchDancers = useCallback(async (schoolId: string) => {
    ...
    if (data) {
      setDancers(data);
      setSchoolCode(schoolId.split('-')[0].toUpperCase()); // mock code
    }
  ...
  const calculateTotal = () => {
    const base = 15 + 150; // Adesão + Inscrição
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col md:flex-row items-start justify-between gap-4">
        ...
        <div className="flex bg-zinc-900 border border-zinc-800 rounded-lg p-1 items-center gap-2">
            ...
            {selectedSchool && selectedSchool !== 'create' && selectedSchool !== 'none' && (
              <Button 
                variant="ghost" 
                size="icon" 
      ...
      {selectedSchool && selectedSchool !== 'create' && selectedSchool !== 'none' && (
        <div className="bg-purple-900/10 border border-purple-500/20 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Right Column: Financial Split */}
        <div className="space-y-6">
          <Card className="bg-[#0A0A0A] border-zinc-700 relative overflow-hidden group">
            ...
            <CardHeader>
              ...
              <CardDescription className="text-zinc-400 text-xs">A plataforma enviará a cobrança unificada automaticamente (Split) para cada bailarino. Adicione aqui as taxas da escola.</CardDescription>
            </CardHeader>
